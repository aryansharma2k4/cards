...
import { leaveCasino } from '../games/casino'
...
export function LeaveDialog() {
  ...
  const poker = useGame((s) => s.screen === 'table')
  const stack = useGame((s) => (s.screen === 'table' ? (s.seats[0]?.stack ?? 0) : (s.casino?.stack ?? 0)))
  return (
    <Dialog open={open} onClose={close} title="Leave the table?">
      <p className="mb-5 text-[15px] text-[#efe6cf]">
        Your {formatMoney(stack)} goes back to your bankroll.{poker && ' Chips already in the pot this hand stay there.'}
      </p>
      <div className="flex justify-end gap-3">
        <Button variant="wood" onClick={close} autoFocus>
          Keep playing
        </Button>
        <Button variant="danger" onClick={() => (poker ? game.leave() : leaveCasino())}>
          Leave
        </Button>
      </div>
    </Dialog>
